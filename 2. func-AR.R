...
runAR=function(Y,indice,lag,type="fixed"){
  ...
  aux=embed(Y2,12+lag) #create 4 lags + forecast horizon shift (=lag option)
  ...
  ###############   added in aic   ################
  if(type=="aic"){ #if selection on AIC
    aa=Inf #initialize the "best AIC" at a huge number
    for(i in seq(1,ncol(X),1)){ #try for every lag length 1:12
      m=lm(y~X[,1:i]) #estimate AR(i) by OLS 
      crit=AIC(m) #retrieve AIC
      if(crit<aa){ #if AIC improved
        aa=crit #updated the new best AIC
        model=m #save the model object
        ar.coef=coef(model) #save coefficients
        #ar.coef=ar.coef[-length(ar.coef)] #remove the dummy coefficient
      }
...
ar.rolling.window=function(Y,nprev,indice=1,lag=1,type="fixed"){
  ...
  save.coef=matrix(NA,nprev,13) #blank matrix for coefficients at each iteration
